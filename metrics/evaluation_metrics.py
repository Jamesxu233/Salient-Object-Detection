...
class SegmentationMetric(object):
    ...
    def meanIntersectionOverUnion(self):
        ...
        return mIoU
    def dice(self):
        # Intersection = TP Union = TP + FP + FN+ TN
        # Dice = TP / (TP + FP + FN + TN)
        intersection = np.diag(self.confusionMatrix) # 取对角元素的值，返回列表
        union = self.confusionMatrix.sum()
...
if __name__ == '__main__':
    imgPredict = glob.glob("E:/saved_models/ablation experiment/ssim/crop/*.png")# 可直接换成预测图片
    imgLabel = glob.glob("E:/data/CRACK dataset/CRACK500/testcrop/label/*") # 可直接换成标注图片
    metric = SegmentationMetric(2) # 3表示有3个分类，有几个分类就填几
    for i in range (len(imgPredict)):
        img = Image.open(imgPredict[i]).convert('P').resize((224,224))
        label = Image.open(imgLabel[i]).convert('P').resize((224,224))
        img = np.array(img,dtype=np.uint8)
        img[img>=1]=1
        label = np.array(label,dtype=np.uint8)
        label[label>=1]=1
        metric.addBatch(img, label)
